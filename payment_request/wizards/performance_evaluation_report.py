...
class PerformanceReport(models.TransientModel): 
    ...
    def action_generate_performance_excel(self):
        ...
        for vehicle, data in vehicles.items():  
            ...
            sheet.write(row, 9, f'{"{:.2f}".format(result.get(truck, {}).get('all_amount', 0) / result_total * 100) }%', content_format)
        ...
        sheet.write(row, 7, all_fuel/all_distance, content_format4)
        ...
        for month in  months : 
            ...
            sheet.write(row, 10,f'{"{:.2f}".format(all_result_not_done[month]['count'] / all_picks * 100)}%', content_format4) 
            sheet.write(row, 11, all_result[month]['count'], content_format4)
            sheet.write(row, 12, '', content_format4)
            sheet.write(row, 13, f'{"{:.2f}".format(all_result[month]['count'] / all_picks * 100)}%', content_format4) 
            row+=1
