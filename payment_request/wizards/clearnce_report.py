...
class ClearnceReport(models.TransientModel): 
    ...
    def action_generate_clearnce_excel(self):
        ...
        for rec in operating_costs:
            if rec.state == "paid":    
                if rec.vehicle_id not in operating_cost:
                ...
                # operating_cost[rec.vehicle_id]['amount']+=rec.custody_line_ids.amount
        ...
        # sheet.write_merge(0, new_row, new_row, 8, 'مؤشرات تقييم الاداء الشهري لخركة وتشغيل الدفارات للفترة', main_heading)
        new_row+=1
        sheet.write(new_row, 0, "رقم الدفار", heading)
        sheet.write(new_row, 1, "اسم السائق", heading)
        sheet.write(new_row, 2, "اجمالي اللترات", heading)
        sheet.write(new_row, 3, """اجمالي تكلفة
         الوقود""", heading)
        sheet.write(new_row, 4, """اجمالي تكلفة
  التشغيل لكل 
دفار""", heading)
        sheet.write(new_row, 5, "عداد الكيلو", heading)
        sheet.write(new_row, 6, """معدل الكفاءة والاستهلاك (كم/لتر)""", heading)
        sheet.write(new_row, 7, """نسبة التشغيل
         الفعلي""", heading)
        sheet.write(new_row, 8, """تكلفة الصيانة
         لكل كيلومتر""", heading)
        sheet.write(new_row, 9, """نسبة استهلاك 
        ...
        for vehicle, data in vehicles.items():  
            ...
            sheet.write(new_row, 0, data['license_plate'], content_format)
            sheet.write(new_row, 1, data['driver'], content_format)
            sheet.write(new_row, 2, data['fuel'], content_format)
            sheet.write(new_row, 3, data['amount'], content_format)
            sheet.write(new_row, 4, operating_cost[license_plate]['cost'], content_format) 
            sheet.write(new_row, 5, distance ,content_format)
            sheet.write(new_row, 6, distance / data['fuel'], content_format)
            # sheet.write(new_row, 7, "نسبة التشغيل الفعلي", heading)
            # sheet.write(new_row, 8, "تكلفة الصيانة لكل كيلومتر", heading)
            # sheet.write(new_row, 9, "نسبة استهلاك الوقود من الميزانية", heading)
            new_row+=1
