...
class MovementOperationReport(models.TransientModel): 
    ...
    def action_generate_movement_operation_excel(self):
        ...
        sheet.write(1, 9, "الصيانة", heading)
        sheet.write(1, 10, "مغسلة", heading)
        sheet.write(1, 11, "بنشر", heading)
        sheet.write(1, 12, "مخالفات حكومية", heading)
        sheet.write(1, 13, "تأمين", heading)
        sheet.write(1, 14, "تفتيش شهري", heading)
        sheet.write(1, 15, "أخرى", heading) 
        sheet.write(1, 16, "ملاحظات", heading)
        ...
        for rec in operating_costs:
            if rec.state == "paid":    
                ...
                if rec.custody_type == 'other':
                    sheet.write(row, 15, rec.total_amount, content_format)  
        ...
        sheet.write(row, 9, total_maintenance , currency_format)
        sheet.write(row, 10, total_car_wash , currency_format)
        sheet.write(row, 11, total_car_tire_repair , currency_format)
        sheet.write(row, 12, total_violations , currency_format) 
