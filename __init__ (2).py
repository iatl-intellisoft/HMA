...
class TopProductsReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for i, rec in enumerate(grouped, start=1):
            ...
            # أول إذن استلام (وارد) للمنتج خلال نفس الفترة
            incoming_move = self.env['stock.move'].search([
            ...
            arrival_qty = 0
            if incoming_move:
                arrival_date = incoming_move.date.strftime('%d/%m/%Y')
                arrival_qty = incoming_move.product_uom_qty
